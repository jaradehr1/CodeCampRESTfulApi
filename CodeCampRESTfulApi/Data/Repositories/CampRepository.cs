...
namespace CodeCampRESTfulApi.Data.Repository {
    public class CampRepository : ICampRepository {
        private readonly CodeCampContext _context;
        private readonly ILogger<CampRepository> _logger;

        public CampRepository(CodeCampContext context, ILogger<CampRepository> logger) {
            _context = context;
            _logger = logger;
        }
        ...
        public async Task<Talk[]> GetTalksByTopicMonikerAsync(string moniker, string subject, bool includeSpeakers = false) {
            _logger.LogInformation($"Getting all Talks related to subject for a Camp");
            ...
            query = query
                .Where(t => t.Camp.Moniker == moniker && (t.Title.Contains(subject) || (t.Abstract.Contains(subject))))
                .OrderByDescending(t => t.Title);
        ...
        public async Task<Talk[]> GetAllTalksAsync() {
            _logger.LogInformation($"Getting Talks");
        ...
        public async Task<Speaker[]> GetAllSpeakersAsync() {
            _logger.LogInformation($"Getting Speakers");
        ...
        public async Task<Speaker> GetSpeakerByNameAsync (string speakerFirstName, string speakerLastName, string speakerMiddleName) {
            _logger.LogInformation($"Getting Speaker By Name");

            var query = _context.Speakers
              .Where(t => t.FirstName == speakerFirstName && t.LastName == speakerLastName && t.MiddleName == speakerMiddleName);
